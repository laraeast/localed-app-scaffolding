...
trait HasValidation
{
    ...
    {
        $this->json($method, $url, [$field => null])
    ...
    {
        $this->json($method, $url, [$field => 'string'])
            ->assertJsonValidationErrors([$field]);

        $this->json($method, $url, [$field => 2])
            ->assertJsonValidationErrors([$field]);

        $this->json($method, $url, [$field => '1'])
            ->assertJsonMissingValidationErrors([$field]);

        $this->json($method, $url, [$field => '0'])
            ->assertJsonMissingValidationErrors([$field]);

        $this->json($method, $url, [$field => true])
            ->assertJsonMissingValidationErrors([$field]);

        $this->json($method, $url, [$field => false])
            ->assertJsonMissingValidationErrors([$field]);
    ...
    {
        $val = factory($model)->create()->id;

        $this->json($method, $url, [$field => 'invalid'])
            ->assertJsonValidationErrors([$field]);

        $this->json($method, $url, [$field => $val])
            ->assertJsonMissingValidationErrors([$field]);
    ...
    {
        $this->json($method, $url, [$field => 1.5])
            ->assertJsonMissingValidationErrors([$field]);

        $this->json($method, $url, [$field => 'wrong value'])
            ->assertJsonValidationErrors([$field]);
    ...
    {
        $this->json($method, $url, [$field => 'dummy string'])
            ->assertJsonValidationErrors([$field]);

        $this->json($method, $url, [$field => $this->dummyBase64Image()])
            ->assertJsonMissingValidationErrors([$field]);
